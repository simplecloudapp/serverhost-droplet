...
object ConditionalAction : YamlAction<ConditionalActionData> {
    override fun exec(ctx: YamlActionContext, data: ConditionalActionData) {
        ...
        val actions = ctx.retrieve<Map<String, List<Pair<YamlActionTypes, Any>>>>("actions")
            ?: throw NullPointerException("actions context is required but was not found")
        val parsedFirst = placeholders.parse(data.first)
        val parsedSecond = placeholders.parse(data.second)
        if (!ConditionalActionMatchers.valueOf(data.matcher.uppercase()).matcher.match(
                parsedFirst,
                parsedSecond
            )
        ) throw Exception("Assertation ${data.matcher} failed: $parsedFirst, $parsedSecond")
        if (ParallelActionGroupExecutor(ctx, actions).execute(data.executes).isNotEmpty()) {
